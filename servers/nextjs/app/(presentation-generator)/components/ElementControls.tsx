...
const SHADOWS: { labelKey: string; value: string }[] = [
  { labelKey: "ep.el.shadow.none", value: "" },
  { labelKey: "ep.el.shadow.soft", value: "0 2px 8px rgba(0,0,0,0.12)" },
  { labelKey: "ep.el.shadow.medium", value: "0 6px 18px rgba(0,0,0,0.18)" },
  { labelKey: "ep.el.shadow.strong", value: "0 14px 40px rgba(0,0,0,0.28)" },
];
...
const SwatchRow: React.FC<{
  ...
  onClear?: () => void;
}> = ({ label, clearLabel, onPick, onClear }) => (
  <div className="mb-3">
    ...
    <div className="flex flex-wrap items-center gap-2">
      ...
      {onClear && (
        <button onClick={onClear} className="ml-1 text-xs text-neutral-500 hover:text-neutral-800">{clearLabel}</button>
      )}
    </div>
  </div>
);
...
const ElementControls: React.FC<{ slideIndex: number; path: string }> = ({
  ...
  const chartKeys: string[] = useMemo(() => {
    if (chartData) {
      ...
      const rows = Array.isArray(chartData.data)
        ...
        : Array.isArray(chartData)
          ? chartData
          : [];
      if (rows.length) return rows.slice(0, 8).map((r: any, i: number) => r?.name || t("ep.el.colorN", { n: i + 1 }));
    }
    const el = resolveEl();
    const legend = el
    ...
    return [1, 2, 3, 4].map((n) => t("ep.el.colorN", { n }));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [chartData, slideIndex, path, t]);
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-base font-semibold text-[#191919]">
            {targetPaths.length > 1
              ? t("ep.el.titleMulti", { n: targetPaths.length })
              : overlayIndex != null
                ? t("ep.el.titleIcon")
                : t("ep.el.title")}
          </span>
        ...
        <div className="flex items-center gap-1">
          {overlayIndex != null && (
            <button
              ...
              title={t("ep.el.deleteIcon")}
              className="rounded-md p-1.5 text-red-500 hover:bg-red-50"
            >
              <Trash2 className="h-4 w-4" />
            </button>
          )}
          <button onClick={() => targetPaths.forEach((tp) => dispatch(clearStyleOverride({ slideIndex, elementPath: tp })))} title={t("ep.common.reset")} className="rounded-md p-1.5 text-neutral-500 hover:bg-neutral-100"><RotateCcw className="h-4 w-4" /></button>
          <button onClick={() => setElement(null)} title={t("ep.common.close")} className="rounded-md p-1.5 text-neutral-500 hover:bg-neutral-100"><X className="h-4 w-4" /></button>
        </div>
      </div>

      <div className="min-h-0 flex-1 overflow-y-auto pr-1 font-inter">
        <SwatchRow label={t("ep.el.bg")} clearLabel={t("ep.common.remove")} onPick={(c) => patch({ backgroundColor: c })} onClear={() => patch({ backgroundColor: undefined })} />
        <SwatchRow label={t("ep.el.text")} onPick={(c) => patch({ textColor: c })} />
        <SwatchRow label={t("ep.el.border")} clearLabel={t("ep.common.remove")} onPick={(c) => patch({ borderColor: c, borderWidth: (override.borderWidth ?? 0) > 0 ? override.borderWidth : 2, borderStyle: "solid" })} onClear={() => patch({ borderWidth: 0 })} />

        <div className="mb-3 flex items-center gap-2">
          <span className="w-16 text-xs text-neutral-500">{t("ep.el.borderWidth")}</span>
        ...
        <p className="mb-1.5 text-xs font-medium text-neutral-500">{t("ep.el.shadow")}</p>
        <div className="mb-3 flex flex-wrap gap-1.5">
          {SHADOWS.map((s) => (
            <button key={s.labelKey} onClick={() => patch({ boxShadow: s.value || undefined })} className={`rounded-md border px-2.5 py-1.5 text-xs ${(override.boxShadow || "") === s.value ? "border-[#e25a4e] bg-[#e25a4e]/10 text-[#e25a4e]" : "border-neutral-200 text-neutral-600 hover:bg-neutral-50"}`}>{t(s.labelKey)}</button>
          ))}
        </div>

        <div className="mb-3 flex items-center gap-2">
          <span className="w-16 text-xs text-neutral-500">{t("ep.el.radius")}</span>
        ...
        <div className="mb-1.5 flex items-center justify-between">
          <span className="text-xs font-medium text-neutral-500">{t("ep.el.size")}</span>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setAspectLocked(!aspectLocked)}
              title={aspectLocked ? t("ep.el.aspectLocked") : t("ep.el.aspectFree")}
        ...
        {!aspectLocked && (
          <div className="mb-3 flex items-center gap-2">
            <span className="w-16 text-xs text-neutral-500">{t("ep.el.height")}</span>
        ...
        <p className="mb-1.5 text-xs font-medium text-neutral-500">{t("ep.el.alignSlide")}</p>
        <div className="mb-3 flex items-center gap-1.5">
          {([
            { icon: AlignStartVertical, act: () => align("left", null), tk: "ep.align.left" },
            { icon: AlignCenterVertical, act: () => align("center", null), tk: "ep.align.center" },
            { icon: AlignEndVertical, act: () => align("right", null), tk: "ep.align.right" },
          ] as const).map((b, i) => (
            <button key={i} onClick={b.act} title={t(b.tk)} className="flex h-8 w-8 items-center justify-center rounded-md border border-neutral-200 text-neutral-600 hover:bg-neutral-50">
              <b.icon className="h-4 w-4" />
            </button>
          ))}
          <span className="mx-1 h-5 w-px bg-neutral-200" />
          {([
            { icon: AlignStartHorizontal, act: () => align(null, "top"), tk: "ep.align.top" },
            { icon: AlignCenterHorizontal, act: () => align(null, "middle"), tk: "ep.align.middle" },
            { icon: AlignEndHorizontal, act: () => align(null, "bottom"), tk: "ep.align.bottom" },
          ] as const).map((b, i) => (
            <button key={i} onClick={b.act} title={t(b.tk)} className="flex h-8 w-8 items-center justify-center rounded-md border border-neutral-200 text-neutral-600 hover:bg-neutral-50">
              <b.icon className="h-4 w-4" />
            </button>
        ...
        <p className="mb-1.5 text-xs font-medium text-neutral-500">{t("ep.el.alignText")}</p>
        <div className="mb-3 flex items-center gap-1.5">
          {([["left", "ep.align.left"], ["center", "ep.align.center"], ["right", "ep.align.right"]] as const).map(([v, tk]) => (
            <button key={v} onClick={() => patch({ textAlign: override.textAlign === v ? undefined : v })} title={t(tk)} className={`h-8 rounded-md border px-2 text-xs ${override.textAlign === v ? "border-[#e25a4e] bg-[#e25a4e]/10 text-[#e25a4e]" : "border-neutral-200 text-neutral-600 hover:bg-neutral-50"}`}>
              {v === "left" ? "⇤" : v === "center" ? "⇹" : "⇥"}
            </button>
          ))}
          <span className="mx-1 h-5 w-px bg-neutral-200" />
          {([["top", "ep.align.top"], ["middle", "ep.align.middle"], ["bottom", "ep.align.bottom"]] as const).map(([v, tk]) => (
            <button key={v} onClick={() => patch({ vAlign: override.vAlign === v ? undefined : v })} title={t(tk)} className={`h-8 rounded-md border px-2 text-xs ${override.vAlign === v ? "border-[#e25a4e] bg-[#e25a4e]/10 text-[#e25a4e]" : "border-neutral-200 text-neutral-600 hover:bg-neutral-50"}`}>
              {v === "top" ? "⤒" : v === "middle" ? "⇳" : "⤓"}
            </button>
          ))}
        </div>
        ...
        <div className="mb-3 flex items-center gap-2">
          <span className="w-16 text-xs text-neutral-500">{t("ep.el.rotation")}</span>
        ...
        <div className="mb-3 flex items-center gap-2">
          <span className="w-16 text-xs text-neutral-500">{t("ep.el.opacity")}</span>
        ...
        <div className="mb-3 flex items-center gap-2">
          <span className="w-16 text-xs text-neutral-500">{t("ep.el.order")}</span>
          <button onClick={() => patch({ zIndex: 50 })} className={`flex items-center gap-1.5 rounded-md border px-2.5 py-1.5 text-xs ${override.zIndex === 50 ? "border-[#e25a4e] bg-[#e25a4e]/10 text-[#e25a4e]" : "border-neutral-200 text-neutral-600 hover:bg-neutral-50"}`}>
            <BringToFront className="h-3.5 w-3.5" /> {t("ep.el.front")}
          </button>
          <button onClick={() => patch({ zIndex: -1 })} className={`flex items-center gap-1.5 rounded-md border px-2.5 py-1.5 text-xs ${override.zIndex === -1 ? "border-[#e25a4e] bg-[#e25a4e]/10 text-[#e25a4e]" : "border-neutral-200 text-neutral-600 hover:bg-neutral-50"}`}>
            <SendToBack className="h-3.5 w-3.5" /> {t("ep.el.back")}
          </button>
          {override.zIndex != null && (
            <button onClick={() => patch({ zIndex: undefined })} className="text-xs text-neutral-500 hover:text-neutral-800">{t("ep.common.auto")}</button>
          )}
        </div>

        {isChart && chartKeys.length > 0 && (
          <div className="mb-3">
            <p className="mb-1.5 text-xs font-medium text-neutral-500">{t("ep.el.chartColors")}</p>
            {chartKeys.map((k, i) => (
              <div key={i} className="mb-1.5 flex items-center gap-2">
                ...
                {graphColors[i] && (
                  <button onClick={() => dispatch(setGraphColor({ slideIndex, index: i, color: null }))} className="text-[11px] text-neutral-400 hover:text-neutral-700">{t("ep.common.auto")}</button>
                )}
              </div>
            ))}
          </div>
        ...
        <p className="text-[11px] text-neutral-400">{t("ep.el.tip")}</p>
      </div>
    </div>
  );
};
