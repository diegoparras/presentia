...
const suggestions: { id: string; icon: ReactNode; suggestionKey: string }[] = [
  {
    id: "generate",
    icon: (
      <svg
    ...
    suggestionKey: "ed.chat.sugg.generate",
  },
  {
    id: "improve",
    icon: (
    ...
    suggestionKey: "ed.chat.sugg.improve",
  },
  {
    id: "rewrite",
    icon: (
    ...
    suggestionKey: "ed.chat.sugg.rewrite",
  },
  {
    id: "notes",
    icon: (
    ...
    suggestionKey: "ed.chat.sugg.notes",
  },
];

const outlineQuickPromptKeys = [
  "ed.chat.qp.expand",
  "ed.chat.qp.shorten",
  "ed.chat.qp.reorder",
  "ed.chat.qp.merge",
  "ed.chat.qp.split",
  "ed.chat.qp.conclusion",
  "ed.chat.qp.introduction",
];
...
const TOOL_LABEL_KEYS: Record<string, string> = {
  getPresentationOutline: "ed.tool.outlineReader",
  getOutlineDraft: "ed.tool.outlineDraftReader",
  addOutline: "ed.tool.outlineAdder",
  updateOutline: "ed.tool.outlineEditor",
  deleteOutline: "ed.tool.outlineRemover",
  moveOutline: "ed.tool.outlineReorderer",
  searchSlides: "ed.tool.slideSearch",
  getSlideAtIndex: "ed.tool.slideReader",
  getPresentationThemeCatalog: "ed.tool.themeCatalog",
  getAvailableLayouts: "ed.tool.layoutFinder",
  getContentSchemaFromLayoutId: "ed.tool.schemaChecker",
  generateAssets: "ed.tool.assetGenerator",
  saveSlide: "ed.tool.slideSaver",
  deleteSlide: "ed.tool.slideRemover",
  setPresentationTheme: "ed.tool.themeApplier",
};
...
const getToolLabel = (t: Translate, tool?: string) => {
  if (!tool) {
    return "";
  }
...
const humanizeTraceMessage = (t: Translate, message: string, tool?: string) => {
  ...
  if (lower === "reading deck context") {
    return t("ed.trace.reviewingContext");
  }
  if (lower === "reading the presentation outline") {
    return t("ed.trace.readingOutline");
  }
  if (lower === "reading the outline draft") {
    return t("ed.trace.readingDraft");
  }
  if (lower === "adding an outline slide") {
    return t("ed.trace.addingOutline");
  }
  if (lower === "updating the outline slide") {
    return t("ed.trace.updatingOutline");
  }
  if (lower === "deleting the outline slide") {
    return t("ed.trace.deletingOutline");
  }
  if (lower === "reordering outline slides") {
    return t("ed.trace.reorderingOutline");
  }
  if (lower === "searching relevant slides") {
    return t("ed.trace.searchingSlides");
  }
  if (lower === "opening the requested slide") {
    return t("ed.trace.openingSlide");
  }
  if (lower === "checking available themes") {
    return t("ed.trace.checkingThemes");
  }
  if (lower === "checking available layouts") {
    return t("ed.trace.checkingLayouts");
  }
  if (lower === "checking the layout schema") {
    return t("ed.trace.checkingSchema");
  }
  if (lower === "generating slide assets") {
    return t("ed.trace.generatingAssets");
  }
  if (lower === "saving the slide") {
    return t("ed.trace.savingSlide");
  }
  if (lower === "deleting the slide") {
    return t("ed.trace.deletingSlide");
  }
  if (lower === "applying presentation theme") {
    return t("ed.trace.applyingTheme");
  }
  if (lower.startsWith("using tools:")) {
    const toolNames = trimmed
      ...
      .filter(Boolean)
      .map((entry) => getToolLabel(t, entry));
    if (toolNames.length === 0) {
      return t("ed.trace.planningSteps");
    }
    return t("ed.trace.planningTools", { tools: toolNames.join(", ") });
  }
  if (lower.includes("found requested data")) {
    if (tool === "getSlideAtIndex") {
      return t("ed.trace.foundSlide");
    }
    if (tool === "getPresentationOutline") {
      return t("ed.trace.foundOutline");
    }
    return t("ed.trace.foundInfo");
  }
...
const formatTraceActivity = (
  ...
  if (typeof trace.message === "string" && trace.message.trim().length > 0) {
    return {
      label: humanizeTraceMessage(t, trace.message, trace.tool),
      kind: trace.kind,
      round: trace.round,
      tool: trace.tool,
      state:
  ...
  if (trace.tool && trace.status === "start") {
    return {
      label: t("ed.trace.running", { tool: getToolLabel(t, trace.tool) }),
      kind: trace.kind,
      round: trace.round,
      tool: trace.tool,
      state: "running",
  ...
  if (trace.tool && trace.status === "success") {
    return {
      label: t("ed.trace.completed", { tool: getToolLabel(t, trace.tool) }),
      kind: trace.kind,
      round: trace.round,
      tool: trace.tool,
      state: "success",
  ...
  if (trace.tool && trace.status === "error") {
    return {
      label: t("ed.trace.failed", { tool: getToolLabel(t, trace.tool) }),
      kind: trace.kind,
      round: trace.round,
      tool: trace.tool,
      state: "error",
  ...
  if (
    ...
    return {
      label: t("ed.trace.planningTools", {
        tools: trace.tools.map((tool) => getToolLabel(t, tool)).join(", "),
      }),
      kind: trace.kind,
      round: trace.round,
      state: "info",
...
const Chat = ({
    ...
    const run = async () => {
      try {
        ...
        const detail =
          error instanceof Error
            ? error.message
            : t("ed.chat.err.loadDetail");
        notify.error(t("ed.chat.err.load"), detail);
      } finally {
        if (!cancelled) {
  ...
  const refreshPresentationIncrementally = useCallback(async () => {
    ...
    try {
      ...
      notify.error(t("ed.chat.err.refresh"), t("ed.chat.err.refreshSaved"));
    } finally {
  ...
  const refreshPresentationIfNeeded = async (toolCalls: string[]) => {
    ...
    try {
      ...
      notify.error(t("ed.chat.err.refresh"), t("ed.chat.err.refreshDone"));
    }
  };
  ...
  const submitMessage = async (rawMessage: string) => {
    ...
    if (!presentationId) {
      notify.error(
        t("ed.chat.err.notReady"),
        t("ed.chat.err.notReadyDesc")
      );
      return;
    }
    ...
    try {
      ...
      const response = await PresentationChatApi.streamMessage(
        ...
        },
        {
          ...
          onTrace: (trace) => {
            ...
            const traceActivity = formatTraceActivity(t, trace);
            if (!traceActivity) {
              return;
            }
      ...
      const message =
        error instanceof Error ? error.message : t("ed.chat.err.send");
      ...
      notify.error(t("ed.chat.err.chat"), message);
    } finally {
      setActiveMutationToolCount(0);
    ...
    <div className={cn("flex h-full w-full flex-col bg-white", "")}>
      <div className="flex items-center justify-between px-4 pt-8">
        <div className="flex items-center gap-2">
          ...
          {isSending && (
            <span className="inline-flex items-center gap-1 rounded-full bg-[#FBEDEA] px-2 py-0.5 text-[10px] font-medium text-[#c9473c]">
        ...
        {!isOutlineVariant && (
          <button
            ...
            aria-label={t("ed.chat.reset")}
            title={t("ed.chat.reset")}
          >
            ...
            {isOutlineVariant ? (
              <div>
                <h4 className="mb-2 text-[10px] font-normal leading-[15px] tracking-[0.367px] text-[#99A1AF]">
                  {t("ed.chat.quickPrompts")}
                </h4>
                <div className="flex flex-wrap gap-2">
                  {outlineQuickPromptKeys.map((promptKey) => (
                    <button
                      key={promptKey}
                      type="button"
                ...
                <div className="flex flex-col gap-1.5">
                  {suggestions.map((suggestion) => (
                    <button
                      key={suggestion.id}
                      type="button"
                      onClick={() => applyPrompt(t(suggestion.suggestionKey))}
                            ...
                            >
                              {activityItem.tool && (
                                <span className="mr-1 text-[#475467]">
                                  {getToolLabel(t, activityItem.tool)}:
                                </span>
                              )}
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2 border border-[#EDEEEF] bg-white px-3 py-1 rounded-[64px]">
            <button
              ...
              aria-label={t("ed.chat.attach")}
              title={t("ed.chat.attachSoon")}
            >
            ...
            <ToolTip
              content={
                isFollowAgentEnabled
                  ? t("ed.chat.follow.disable")
                  : t("ed.chat.follow.enable")
              }
            >
              <button
                type="button"
                ...
                >
                  <g clipPath="url(#clip0_6216_326)">
                    <path
                      ...
                      stroke={isFollowAgentEnabled ? "#e25a4e" : "#000000"}
