...
Estado: implementado (MVP). Piezas: `services/dataset_service.py` (parser CSV/TSV/JSON a forma canónica, límite `DATASET_MAX_ROWS`), `utils/chart_data_guard.py` (conjunto de valores permitidos con agregados de columna completa, extracción recursiva de números de `chartData`, tolerancia de redondeo), `utils/llm_calls/generate_slide_content_with_data.py` (wrapper con hasta 3 reintentos con feedback y rechazo final), endpoint `POST /presentation/generate-from-data` en `api/v1/ppt/endpoints/data_presentations.py`, columna `dataset` en `presentations` (migración `3e7a91c4d2b8`) y campo aditivo en `GeneratePresentationRequest`. Si Anonimal está activo, la tabla que ve el LLM se anonimiza una vez en el endpoint (fail-closed); las filas crudas quedan en el host y solo alimentan el conjunto de validación numérica. Pendientes como incrementos: agregación por grupos, formatos XLSX, e integración del dataset al wizard de la UI.
...
Estado: implementado. Piezas: `utils/llm_client.py` (proxy instrumentado; los ocho imports de `get_client` del backend apuntan ahí), `services/llm_usage_service.py` (contextvar de atribución, extractor de usage tolerante a formas — el contrato con `llmai` queda fijado por tests —, buffer thread-safe con flush transaccional y reintento), `constants/llm_pricing.py` (catálogo versionado, precios de Anthropic verificados contra la documentación oficial), tabla `llm_usage_events` (migración `6f4b2a8c1d95`), router `usage` con totales por presentación, desglose por etapa/slide/modelo y comparativa por proveedor, y la página Costos en el dashboard. Se controla con `LLM_USAGE_TRACKING` (activa por defecto: no tiene efectos externos y el panel vacío sería confuso; apagarla devuelve el comportamiento vanilla de descartar el usage).
